...
st.set_page_config(page_title="VitalSeconds • Secure Upload", layout="centered")
...
if uploaded_file is not None:
    ...
    try:
        ...
        # Ensure column names are lowercase for mapping compatibility
        df.columns = [c.lower() for c in df.columns]
        
        if 'latitude' in df.columns and 'longitude' in df.columns:
            st.write("---")
            with st.expander("📊 View Ingested Raw Data (Validation Preview)", expanded=True):
                st.metric(label="Total Operational Rows Detected", value=f"{len(df):,}")
    ...
    st.info("Thank you. Our team will process this file and deliver your Perimeter Snapshot within 7–10 business days.")
